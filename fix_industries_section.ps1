...
# Define the proper HTML for Industries We Serve (no escaped quotes)
$newSection = @'
...
# Replace any existing escaped block (if present) and also replace any previous proper block to avoid duplicates
$patternEscaped = "<!-- Industries We Serve Section Start -->.*?<!-- Industries We Serve Section End -->"
$content = [regex]::Replace($content, $patternEscaped, $newSection, "Singleline")

# Write back
Set-Content $path -Value $content -Encoding UTF8
Write-Host "Industries We Serve section fixed and inserted correctly."
