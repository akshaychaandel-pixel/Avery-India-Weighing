...
# Marker where we insert before the footer
$marker = '(?=<footer)'
...
<div class=\"space-y-12\">
  ...
  <div class=\"md:hidden flex justify-center\"><button class=\"border border-primary text-primary font-label-lg px-8 py-3 rounded uppercase tracking-wide hover:bg-primary hover:text-on-primary transition-all\" onclick=\"document.querySelectorAll('#industry-grid > a.hidden').forEach(el => el.classList.remove('hidden')); this.parentElement.classList.add('hidden')\">See More Industries</button></div>
...
Write-Host "Industries We Serve section inserted successfully."
