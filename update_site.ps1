...
$htmlPath = 'D:\Antigravity AI agent\index.html'
...
$html = $html -replace '(?s)<!-- Tank Weighing Deep-Dive Section -->.*?id="tank-weighing-section".*?</section>', $newSection
...
# Inject grid style for the sections
$styleBlock = @"
<style>
...
# Complete Industrial Weighing Solutions Section
$html = $html -replace '(?s)(<!-- Complete Industrial Weighing Solutions Section -->\s*<section)\b', '$1 id="complete-industrial-weighing-section"'

# OUR PROCESS WEIGHING SOLUTIONS
$html = $html -replace '(?s)(<!-- PROCESS WEIGHING SOLUTIONS -->\s*<section[^>]*class="[^"]*)\s*bg-white([^"]*)"', '$1$2" id="process-weighing-section"'
if ($html -notmatch 'id="process-weighing-section"') {
    $html = $html -replace '(?s)(<section[^>]*class="[^"]*bg-white[^"]*"\s*)(?=>\s*<div[^>]*>\s*<div[^>]*>\s*<span[^>]*>OUR PROCESS\s*WEIGHING SOLUTIONS)', '$1 id="process-weighing-section"'
}
